...
_CLIENTE = "apps.autenticacao.gatilho_auditoria._client"
...
class TestDispararGatilho:
    ...
    def test_envia_apenas_realm_e_usuario(self) -> None:
        ...
        with patch(_CLIENTE, cliente):
            resultado = disparar_gatilho(
                "COTIC",
                _USUARIO_ID,
            )
    ...
    def test_nao_propaga_falha_de_comunicacao(
        self,
    ) -> None:
    ...
    def test_considera_recusado_status_diferente_de_202(
        self,
    ) -> None:
...
class TestDispararGatilhoPorLogin:
    ...
    def test_resolve_conta_e_dispara(
        self,
        settings: Any,
    ) -> None:
        ...
        admin = MagicMock()
        admin.obter_dados_usuario.return_value = {
            "kc_user_id": _USUARIO_ID,
        }
        ...
        with (
            ...
            patch(_CLIENTE, cliente),
        ):
            resultado = disparar_gatilho_por_login("1234567")

        assert resultado is True

        admin.obter_dados_usuario.assert_called_once_with("1234567")
    ...
    def test_nao_dispara_quando_login_nao_existe(
        self,
    ) -> None:
        ...
        admin.obter_dados_usuario.assert_called_once_with("inexistente")

        cliente.post.assert_not_called()

    def test_nao_propaga_falha_na_resolucao_da_conta(
        self,
    ) -> None:

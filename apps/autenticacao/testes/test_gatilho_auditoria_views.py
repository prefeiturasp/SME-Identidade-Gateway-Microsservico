...
_AUDIT_MS_CLIENT = "apps.autenticacao.gatilho_auditoria._client"
...
class TestResilienciaAoGatilhoDeAuditoria:
    ...
    def test_login_mantem_200_com_audit_ms_fora_do_ar(
        self,
    ) -> None:
        ...
        cliente_token = _cliente_token_ms_indisponivel()
        cliente_audit = _cliente_audit_indisponivel()

        with (
            patch(_KEYCLOAK_ADMIN_VIEWS) as mock_keycloak,
            patch(_TOKEN_MS_CLIENT, cliente_token),
            patch(_AUDIT_MS_CLIENT, cliente_audit),
        ):
    ...
    def test_login_avisa_a_auditoria_com_realm_e_usuario(
        self,
    ) -> None:
        ...
        cliente_audit.post.return_value = httpx.Response(
            ...
            request=httpx.Request(
                "POST",
                "https://audit-ms/api/v1/gatilho-poll/",
            ),
        )
    ...
    def test_alterar_senha_mantem_200_com_audit_ms_fora_do_ar(
        self,
    ) -> None:
        ...
        cliente_audit.post.assert_called_once()

    def test_alterar_email_mantem_200_com_audit_ms_fora_do_ar(
        self,
    ) -> None:
    ...
    def test_alterar_senha_mantem_204_para_login_inexistente(
        self,
    ) -> None:
        ...
        cliente_audit.post.assert_not_called()
